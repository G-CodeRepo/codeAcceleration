...
int main(int argc, char *argv[]) {
    ...
    int k = 0;
    long int j = 0;	
    omp_set_num_threads(1);	// set number of threads to use
	...
	for (l = 1; l<=count_xy_glbl; l++) {
		...
		for (j=column_array[l]->cell_min; j<=column_array[l]->cell_max; j++) {
			#pragma omp parallel for shared(j) private(k) schedule(dynamic)	// shared(star, R, count, j)
...
void process_SN_end_subsection() {
	...
	for (i=1; i<=ptr_1_range; i++) {
		...
		if(star1[i].sn==1 || star1[i].sn==2) {
			...
			#pragma omp parallel for shared(i) private(j) schedule(dynamic)
...
void process_SN() { 
	...
	for (i=1; i<=ptr_1_range; i++) {
		...
		if(star1[i].sn==1 ||star1[i].sn==2) {
			...
			#pragma omp parallel for shared(i, k) private(j) schedule(dynamic)
			...
			#pragma omp parallel for shared(i, j) private(k) schedule(dynamic)	
	...
	for (l=1; l<=ptr_2_range; l++) {
		if(star2[l].sn==1 || star2[l].sn==2) {
			...
			#pragma omp parallel for shared(l) private(m) schedule(dynamic)
...
int wrap_around_function(float x, float y, float z, float sterilization_distance) {
	...
	arc_length_original=(angle*(2*PI_fn/360))*	sqrt((x*x)+(y*y));		
    ...
    if (arc_length_ratio<=0.5) {
        ...
        wrap_around.y_coord=(sqrt((x*x)+(y*y)))*(sin(outside_angle*(PI_fn/180)));
...
long int running_total(int column) {
	...
	int i = 0;
	//omp_set_num_threads(1);	// set number of threads to use
	//#pragma omp parallel for shared(column, total) private(i) schedule(dynamic)
	for (i=1; i<column; i++) {
	//{
		//#pragma omp critical
		//{
...
void copy_into_star1(long int length) {
	...
	long int i = 0;
	omp_set_num_threads(1);	// set number of threads to use
	#pragma omp parallel for shared(star1) private(i) schedule(dynamic)	// shared(star1)
	for (i=1; i<=length; i++) {	
	{	
		#pragma omp critical
		{
...
void copy_into_star2(long int length) {
	...
	#pragma omp parallel for shared(star2) private(i) schedule(dynamic)
	for (i=1; i<=length; i++) {
	{
		#pragma omp critical
		{
...
void copy_star2_into_star1(long int length) {
	...
	star1 = new star_cell [length+1];
	long int i = 0;
	#pragma omp parallel for shared(star1) private(i) schedule(dynamic)
	for (i=1; i<=length; i++) {
	{
		#pragma omp critical
		{
...
void create_columns() {
	...
	omp_set_num_threads(1);	// set number of threads to use
	int l = 0;
	#pragma omp parallel for shared(column_array) private(l) schedule(dynamic)
...
float Salpeter_IMF() {
	...
	*/
	...
	return (pow(((float)(rg.Random()) * ((pow(m_max_fn,(1-alpha_fn)))-
								  (pow(m_min_fn,(1-alpha_fn))))+
		    pow(m_min_fn,(1-alpha_fn))),(1/(1-alpha_fn))));
...
long int get_star_range_upper(int sn_star) {
    ...
    if (subsect_ptr[cell]->subsection_xy==1) {
		...
		if (subsection==count_z_glbl) {
			...
			return (subsect_ptr[cell]->star_index_max);                  
		} else {
			//upper=subsect_ptr[cell+1]->star_index_max; 
			return (subsect_ptr[cell+1]->star_index_max);   
		...
		return (subsect_ptr[cell]->star_index_max-(running_total(subsect_ptr[cell]->subsection_xy)));               
              
    } else{
		//upper=subsect_ptr[cell+1]->star_index_max-(running_total(column));
		return (subsect_ptr[cell+1]->star_index_max-(running_total(subsect_ptr[cell]->subsection_xy))); 
...
void create_subsections() {
	...
	while(count_xy!=xy_subsections+1){
		...
		//#pragma omp parallel for shared(p_min, p_max) private(count_z) schedule(dynamic)
		//for (count_z = 1; count_z <  z_subsections+1; count_z++) {
		//{
		//	#pragma omp critical
		//	{
