...
// My Global variables
# define NUM_THREADS 1	//omp_get_max_threads();	// set the max number of threads allowed by this processor
...
int main(int argc, char *argv[]) {
	...
	// calculations used inside the loop
	// int radial_dist_calc = 2500/radial_dist_cell_size;
	// float halfCircle = pi_fn/180;	
	// float partialRadialDistance = R/10000;
	...
	for (l = 1; l<=count_xy_glbl; l++) {
		...
		for (j=column_array[l]->cell_min; j<=column_array[l]->cell_max; j++) {
			#pragma omp parallel for shared(star, j) private(k) schedule(dynamic) num_threads(1)
				...
				
			{
					...
					//star[count].x_coord=R*(cos(degree*(halfCircle)));
					//star[count].y_coord=R*(sin(degree*(halfCircle)));
					...
					//star[count].birth_date=generate_birth_date((partialRadialDistance),rg.Random());
					...
					if ((star[count].mass >=type_Ia_mass_lower_fn)&&
						(star[count].mass <type_Ia_mass_upper_fn) && 
						(star[count].death_date<=max_time_fn)){
    ...
    elapsed = ((end.tv_sec*1000000.0 + end.tv_usec) - (start.tv_sec*1000000.0 + start.tv_usec)) / 1000000.00;
...
void process_SN_end_subsection() {
	...
	for (i=1; i<=ptr_1_range; i++) {
		...
		if(star1[i].sn==1 || star1[i].sn==2) {
			...
			#pragma omp parallel for shared(i) private(j) schedule(dynamic) num_threads(10)
                       
			for (j=get_star_range_lower(i); j<=get_star_range_upper(i); j++) {
			{
				...
				distance=(sqrt(((sn_x-star1[j].x_coord)*(sn_x-star1[j].x_coord))+
							 ((sn_y-star1[j].y_coord)*(sn_y-star1[j].y_coord))+
							 ((sn_z-star1[j].z_coord)*(sn_z-star1[j].z_coord))));
								  
				//wrap around distance
				if (wrap_flag==1) {
					distance2=(sqrt(((wrap_around.x_coord-star1[j].x_coord)*(wrap_around.x_coord-star1[j].x_coord))+
				...
				if((distance<=sterilization_distance) && 
				   (birthdate_candidate<deathdate_sn) && 
				...
				if((wrap_flag==1) && 
				   (distance2<=sterilization_distance) && 
				   (birthdate_candidate<deathdate_sn) && 
				   (deathdate_candidate > deathdate_sn) && 
				   (deathdate_sn <=max_time_fn)) {
...
void process_SN() { 
	...
	for (i=1; i<=ptr_1_range; i++) {
		...
		if(star1[i].sn==1 ||star1[i].sn==2) {
			...
			#pragma omp parallel for shared(i, k) private(j) schedule(dynamic) num_threads(1)
			...
			for (j=get_star_range_lower(i); j<=get_star_range_upper(i); j++) {		
			{
				...
				if((distance<=sterilization_distance) && 
				   ...
				   (star1[j].birth_date < star1[i].death_date) && 
				   (star1[j].death_date > star1[i].death_date) && 
				   
				   //(deathdate_sn <=max_time_fn)
				   (star1[i].death_date <=max_time_fn)) {
					...
					//star1[j].sterilization_date=deathdate_sn;
					star1[j].sterilization_date=star1[i].death_date;
			...
			} //end of inner for loop for sterilizations 1->1
			...
			#pragma omp parallel for shared(i, j) private(k) schedule(dynamic) num_threads(1)

			for (k=get_star_range_lower_cell_right(star1[i].cell); k<=get_star_range_upper_cell_right(star1[i].cell); k++){
			{		
				normal_ster_flag=0;
				wrap_ster_flag=0;
				...
				//normal condition
				distance=(sqrt(((sn_x-star2[k].x_coord)*(sn_x-star2[k].x_coord))+
				...
				if (wrap_flag==1) {
					distance2=(sqrt(((wrap_around.x_coord-star2[k].x_coord)*(wrap_around.x_coord-star2[k].x_coord))+
				...
				if((distance<=sterilization_distance) && 
				   ...
				   (star2[k].birth_date < star1[i].death_date) && 
				   (star2[k].death_date > star1[i].death_date) && 
	...
	for (l=1; l<=ptr_2_range; l++) {
		if(star2[l].sn==1 || star2[l].sn==2) {
			...
			#pragma omp parallel for shared(l) private(m) schedule(dynamic) num_threads(1)

			for (m=get_star_range_lower_cell_left(star2[l].cell); m<=get_star_range_upper_cell_left(star2[l].cell); m++){
			{	
				normal_ster_flag=0;
				wrap_ster_flag=0; 
				...
				distance=(sqrt(((sn_x-star1[m].x_coord)*(sn_x-star1[m].x_coord))+
								((sn_y-star1[m].y_coord)*(sn_y-star1[m].y_coord))+
								((sn_z-star1[m].z_coord)*(sn_z-star1[m].z_coord))));
						 
				//wrap around sterilization distance            
				if (wrap_flag==1) {
					distance2=(sqrt(((wrap_around.x_coord-star1[m].x_coord)*(wrap_around.x_coord-star1[m].x_coord))+
				...
				if((distance<=sterilization_distance) && 
				   ...
				   (star1[m].birth_date < star2[l].death_date) && 
				   (star1[m].death_date > star2[l].death_date) &&
				   (star1[m].birth_date < star2[l].death_date) && 
				   (star1[m].death_date > star2[l].death_date) && 
				    
				   //(deathdate_sn <=max_time_fn)
				   (star2[l].death_date <=max_time_fn)) {
...
int wrap_around_function(float x, float y, float z, float sterilization_distance) {
	...
	arc_length_original=(((acos(x/R))*(180/PI_fn))*(2*PI_fn/360))*	sqrt((x*x)+(y*y));		
	arc_length_total=(mw_angle_fn*(2*PI_fn/360))* sqrt((x*x)+(y*y));	
    ...
    if (!((arc_length_total-arc_length_original)<sterilization_distance) && 
		!(arc_length_original<sterilization_distance)) {
		//  cout <<"\nhere1!";                        
		return 0;
    }
    ...
    if (arc_length_ratio<=0.5) {
		...
		wrap_around.x_coord=(sqrt((x*x)+(y*y)))*(cos((mw_angle_fn+(mw_angle_fn*angle))*(PI_fn/180)));                
        wrap_around.y_coord=(sqrt((x*x)+(y*y)))*(sin((mw_angle_fn+(mw_angle_fn*angle))*(PI_fn/180)));
	...
	//wrap_around.x_coord=R*(cos(outside_angle*(PI_fn/180)));
	wrap_around.x_coord=(sqrt((x*x)+(y*y)))*(cos(outside_angle*(PI_fn/180)));

	
	
	//make the y coordinate negative so it's below the x-axis  
	//wrap_around.y_coord=-1*(R*(sin(outside_angle*(PI_fn/180)))); 
	wrap_around.y_coord=-1*((sqrt((x*x)+(y*y)))*(sin(outside_angle*(PI_fn/180)))); 
...
float generate_SFH() {
	...
	return(current_time-eqn7909(rg.Random()));      
...
long int running_total(int column) {
	long int total=0;
	int i = 0;
	//#pragma omp parallel for shared(total) private(i) schedule(dynamic) num_threads(1)
	for (i=1; i<column; i++) {
	//{
		//#pragma omp critical
		//{
...
void copy_into_star1(long int length) {
	...
	/*
	long int i = 0;
	#pragma omp parallel for shared(star1) private(i) schedule(dynamic)	num_threads(1)
	for (i=1; i<=length; i++) {	
	{	
		//#pragma omp critical
		//{
	...
	*/
	...
	//long int a,b,c,d,e,f,g,h,i,j,k,l,m,n,o = 0;
	//#pragma omp parallel sections shared(star1) private(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o)
	//{
		//#pragma omp section
		//{
			long int a = 0;
			#pragma omp parallel for shared(star1) private(a) schedule(dynamic)	num_threads(1)
			for (a=1; a<=length; a++) {	
			{	
				star1[a].cell=star[a].cell;    
			}	// end of pragma
			}	// end of for loop
		//}	// end of section
		//#pragma omp section
		//{	
			long int b = 0;
			#pragma omp parallel for shared(star1) private(b) schedule(dynamic)	num_threads(1)
			for (b=1; b<=length; b++) {	
			{			
				star1[b].mass=star[b].mass; 	
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int c = 0;
			#pragma omp parallel for shared(star1) private(c) schedule(dynamic)	num_threads(1)
			for (c=1; c<=length; c++) {	
			{	
				star1[c].sn=star[c].sn; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int d = 0;
			#pragma omp parallel for shared(star1) private(d) schedule(dynamic)	num_threads(1)
			for (d=1; d<=length; d++) {	
			{	
				star1[d].sterilized=star[d].sterilized; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int e = 0;
			#pragma omp parallel for shared(star1) private(e) schedule(dynamic)	num_threads(1)
			for (e=1; e<=length; e++) {	
			{	
				star1[e].subsection_z=star[e].subsection_z; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int f = 0;
			#pragma omp parallel for shared(star1) private(f) schedule(dynamic)	num_threads(1)
			for (f=1; f<=length; f++) {	
			{	 
				star1[f].x_coord=star[f].x_coord; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int g = 0;
			#pragma omp parallel for shared(star1) private(g) schedule(dynamic)	num_threads(1)
			for (g=1; g<=length; g++) {	
			{	
				star1[g].y_coord=star[g].y_coord; 
			}  // end of pragma
			}  // end of for loop
		//}	// pragma omp section
		//#pragma omp section
		//{
			long int h = 0;
			#pragma omp parallel for shared(star1) private(h) schedule(dynamic)	num_threads(1)
			for (h=1; h<=length; h++) {	
			{	
				star1[h].z_coord=star[h].z_coord;       
			}  // end of pragma
			}  // end of for loop
		//}	// pragma omp section
		//#pragma omp section
		...
		//{
			long int j = 0;
			#pragma omp parallel for shared(star1) private(j) schedule(dynamic)	num_threads(1)
			for (j=1; j<=length; j++) {	
			{	
				star1[j].death_date=star[j].death_date;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int k = 0;
			#pragma omp parallel for shared(star1) private(k) schedule(dynamic)	num_threads(1)
			for (k=1; k<=length; k++) {	
			{
				star1[k].ms_lifetime=star[k].ms_lifetime;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int l = 0;
			#pragma omp parallel for shared(star1) private(l) schedule(dynamic)	num_threads(1)
			for (l=1; l<=length; l++) {	
			{	
				star1[l].sterilization_date=star[l].sterilization_date;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int m = 0;
			#pragma omp parallel for shared(star1) private(m) schedule(dynamic)	num_threads(1)
			for (m=1; m<=length; m++) {	
			{	
				star1[m].sterilized_count=star[m].sterilized_count;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int n = 0;
			#pragma omp parallel for shared(star1) private(n) schedule(dynamic)	num_threads(1)
			for (n=1; n<=length; n++) {	
			{	
				star1[n].sterilized_count=star[n].sterilized_count;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int o = 0;
			#pragma omp parallel for shared(star1) private(o) schedule(dynamic)	num_threads(1)
			for (o=1; o<=length; o++) {	
			{	
				star1[o].sterilization_distance=star[o].sterilization_distance;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
...
void copy_into_star2(long int length) {
	...
	/*
	long int i = 0;
	#pragma omp parallel for shared(star2) private(i) schedule(dynamic) num_threads(1)
	for (i=1; i<=length; i++) {
	{
		//#pragma omp critical
		//{
	...
	*/
	...
	//long int a,b,c,d,e,f,g,h,i,j,k,l,m,n,o = 0;
	//#pragma omp parallel sections shared(star2) private(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o)
	//{
		//#pragma omp section
		//{
			long int a = 0;
			#pragma omp parallel for shared(star2) private(a) schedule(dynamic)	num_threads(1)
			for (a=1; a<=length; a++) {	
			{	
				star2[a].cell=star[a].cell;    
			}	// end of pragma
			}	// end of for loop
		//}	// end of section
		//#pragma omp section
		//{	
			long int b = 0;
			#pragma omp parallel for shared(star2) private(b) schedule(dynamic)	num_threads(1)
			for (b=1; b<=length; b++) {	
			{			
				star2[b].mass=star[b].mass; 	
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int c = 0;
			#pragma omp parallel for shared(star2) private(c) schedule(dynamic)	num_threads(1)
			for (c=1; c<=length; c++) {	
			{	
				star2[c].sn=star[c].sn; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int d = 0;
			#pragma omp parallel for shared(star2) private(d) schedule(dynamic)	num_threads(1)
			for (d=1; d<=length; d++) {	
			{	
				star2[d].sterilized=star[d].sterilized; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int e = 0;
			#pragma omp parallel for shared(star2) private(e) schedule(dynamic)	num_threads(1)
			for (e=1; e<=length; e++) {	
			{	
				star2[e].subsection_z=star[e].subsection_z; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int f = 0;
			#pragma omp parallel for shared(star2) private(f) schedule(dynamic)	num_threads(1)
			for (f=1; f<=length; f++) {	
			{	 
				star2[f].x_coord=star[f].x_coord; 
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int g = 0;
			#pragma omp parallel for shared(star2) private(g) schedule(dynamic)	num_threads(1)
			for (g=1; g<=length; g++) {	
			{	
				star2[g].y_coord=star[g].y_coord; 
			}  // end of pragma
			}  // end of for loop
		//}	// pragma omp section
		//#pragma omp section
		//{
			long int h = 0;
			#pragma omp parallel for shared(star2) private(h) schedule(dynamic)	num_threads(1)
			for (h=1; h<=length; h++) {	
			{	
				star2[h].z_coord=star[h].z_coord;       
			}  // end of pragma
			}  // end of for loop
		//}	// pragma omp section
		//#pragma omp section
		...
		//{
			long int j = 0;
			#pragma omp parallel for shared(star2) private(j) schedule(dynamic)	num_threads(1)
			for (j=1; j<=length; j++) {	
			{	
				star2[j].death_date=star[j].death_date;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int k = 0;
			#pragma omp parallel for shared(star2) private(k) schedule(dynamic)	num_threads(1)
			for (k=1; k<=length; k++) {	
			{
				star2[k].ms_lifetime=star[k].ms_lifetime;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int l = 0;
			#pragma omp parallel for shared(star2) private(l) schedule(dynamic)	num_threads(1)
			for (l=1; l<=length; l++) {	
			{	
				star2[l].sterilization_date=star[l].sterilization_date;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int m = 0;
			#pragma omp parallel for shared(star2) private(m) schedule(dynamic)	num_threads(1)
			for (m=1; m<=length; m++) {	
			{	
				star2[m].sterilized_count=star[m].sterilized_count;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int n = 0;
			#pragma omp parallel for shared(star2) private(n) schedule(dynamic)	num_threads(1)
			for (n=1; n<=length; n++) {	
			{	
				star2[n].sterilized_count=star[n].sterilized_count;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
		//#pragma omp section
		//{
			long int o = 0;
			#pragma omp parallel for shared(star2) private(o) schedule(dynamic)	num_threads(1)
			for (o=1; o<=length; o++) {	
			{	
				star2[o].sterilization_distance=star[o].sterilization_distance;
			}  // end of pragma
			}  // end of for loop
		//}	// end of section
	//}	// end of pragma sections	
...
void create_columns() {
	...
	int l = 0;
	#pragma omp parallel for shared(column_array) private(l) schedule(dynamic) num_threads(1)
	...
	for (l=1; l<=count_xy_glbl; l++) {
	{
		...
		if (l==1) {
			//#pragma omp critical
			//{
			column_array[l]->num_stars = subsect_param_array[column_array[l]->cell_max]->star_index_max;
			//}
		} else {
...
float Salpeter_IMF() {
	...
	//float m_max_fn=m_max;  	// 100 
	//float m_min_fn=m_min;	// 0.08
	float alpha_fn=alpha;	// 2.35
	
	//float stellar_mass;
	//float x = rg.Random();

	/*
	stellar_mass = 	pow(( x* ((pow(m_max_fn,(1-alpha_fn)))-
							 (pow(m_min_fn,(1-alpha_fn))))+
					pow(m_min_fn,(1-alpha_fn))),(1/(1-alpha_fn)));
	return stellar_mass;
	*/
		
	return (pow(( ((float)rg.Random())* ((pow((float)100 , (1 - (float)2.35)))-
										 (pow((float)0.08 , (1 - (float)2.35))))+
			pow((float)0.08,(1 - (float)2.35))),(1/(1-alpha_fn))));	// need 'alpha_fn' to maintain correctness
}
...
long int whole_num_star(float number) {
	...
	if(((float)rg.Random()) <= (number - ((int)number))) {
		//return (whole_num+1);
		return (((long int)((int)number))+1);

	} else {
		//return(whole_num);
		return(((long int)((int)number)));
	}
}
...
long int get_star_range_lower_cell_left(int cell) {
	...
	if(((cell-count_z_glbl)%count_z_glbl)==1) {
		...
		return (subsect_ptr[cell-count_z_glbl]->star_index_min-(running_total((subsect_ptr[cell]->subsection_xy)-1))); 
	} else {
		//stellar_index=subsect_ptr[cell-count_z_glbl-1]->star_index_min-(running_total((subsect_ptr[cell]->subsection_xy)-1));    
		return (subsect_ptr[cell-count_z_glbl-1]->star_index_min-(running_total((subsect_ptr[cell]->subsection_xy)-1)));    
	}
...
long int get_star_range_upper_cell_left(int cell) {
	...
	if(((cell-count_z_glbl)%count_z_glbl)==0) {
		...
		return (subsect_ptr[cell-count_z_glbl]->star_index_max-(running_total((subsect_ptr[cell]->subsection_xy)-1)));
	} else {
		//stellar_index=subsect_ptr[cell-count_z_glbl+1]->star_index_max-(running_total((subsect_ptr[cell]->subsection_xy)-1));    
		return (subsect_ptr[cell-count_z_glbl+1]->star_index_max-(running_total((subsect_ptr[cell]->subsection_xy)-1)));    
	}
	//return(stellar_index);
}
...
long int get_star_range_lower(int sn_star) {
	...
	} else if (subsect_ptr[((long int)star1[sn_star].cell)]->subsection_xy==1) {

		//if its the very first column
        // the very first cell in the first column
        //if (subsection==1){
		if ((star1[sn_star].subsection_z)==1){
			lower=1;              
		} else {
			//if its not the first cell in the column
			//lower=subsect_ptr[cell-1]->star_index_min;  
			lower=subsect_ptr[((long int)star1[sn_star].cell)-1]->star_index_min;   
 
		}
    //} else if (subsection==1 && (subsect_ptr[cell]->subsection_xy>1)) {	
	} else if (((star1[sn_star].subsection_z) == 1) && (subsect_ptr[((long int)star1[sn_star].cell)]->subsection_xy>1)) {
		...
		prev_col_max=running_total(subsect_ptr[((long int)star1[sn_star].cell)]->subsection_xy);
		lower=subsect_ptr[((long int)star1[sn_star].cell)]->star_index_min-prev_col_max;              
    } else {
		//not the first cell
        //prev_col_max=running_total(subsect_ptr[cell]->subsection_xy);
		...
		prev_col_max=running_total(subsect_ptr[((long int)star1[sn_star].cell)]->subsection_xy);
        lower=subsect_ptr[((long int)star1[sn_star].cell)-1]->star_index_min-prev_col_max;  
    }
...
long int get_star_range_upper(int sn_star) {
	...
	if (subsect_ptr[star1[sn_star].cell]->subsection_xy==1) {


		//if (subsection==count_z_glbl) {
		if ((star1[sn_star].subsection_z) == count_z_glbl) {

			//upper=subsect_ptr[cell]->star_index_max; 
			return (subsect_ptr[star1[sn_star].cell]->star_index_max);                  
		} else {
			//upper=subsect_ptr[cell+1]->star_index_max; 
			return (subsect_ptr[(star1[sn_star].cell)+1]->star_index_max);   
  
		}                                
    //} else if (subsection==count_z_glbl) {
	} else if ((star1[sn_star].subsection_z) == count_z_glbl) {
		//if its the last cell- not in the first column
		//upper=subsect_ptr[cell]->star_index_max-(running_total(column)); 
		return (subsect_ptr[star1[sn_star].cell]->star_index_max-(running_total(subsect_ptr[star1[sn_star].cell]->subsection_xy)));               
              
    } else{
		//upper=subsect_ptr[cell+1]->star_index_max-(running_total(column));
		return (subsect_ptr[(star1[sn_star].cell)+1]->star_index_max-(running_total(subsect_ptr[star1[sn_star].cell]->subsection_xy))); 
...
float degree_generator(float deg_min, float deg_max){
	...
	return ((float)rg.Random() + (int32)(rg.IRandomX(deg_min,deg_max-1)));
}
...
float random_star_generator_Z(float z_min, float z_max){
	...
	return ((float)rg.Random() + (int)(rg.IRandom(z_min,z_max-1))); 
}
...
float random_star_generator_R (float deg_min, float deg_max, float r_min, float r_max) {
	...
	//r_float = rg.Random();
	//w=r_float;
	//R=sqrt( w*((r_max*r_max)-(r_min*r_min))+(r_min*r_min));
	...
	return sqrt((float)(rg.Random())*((r_max*r_max)-(r_min*r_min))+(r_min*r_min));
}
...
void create_subsections() {
	...
	//struct subsection_parameters **subsect_param_array = NULL;
	//float mw_radius_fn=mw_radius;	// 15330
	//float mw_height_fn=mw_height;	// 5000
	//float mw_angle_fn=mw_angle;	// 1
	...
	// my version of above
	// int xy_subsections = ceil((float)15330/30);
	//int z_subsections = ceil((float)5000/30);
	...
	float mw_height_real= (int)(ceil((float)5000/30))*30;
	...
	while(count_xy != (((int)ceil((float)15330/30))+1)){
		...
		for (count_z = 1; count_z <  (((int)ceil((float)5000/30))+1); count_z++) {
		{
			...
			subsect_param_array = (struct subsection_parameters **)realloc(subsect_param_array, (count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))+ 1) * sizeof(struct subsection_parameters *));
			...
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]= (struct subsection_parameters*)malloc(sizeof(struct subsection_parameters));
			...
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->subsection_xy=count_xy;
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->subsection_z=count_z;
			...
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->angle_min=0;
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->angle_max=mw_angle
			...
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_max=mw_height_above_mp;

			
			//subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->z_min=mw_height_above_mp-subsection_slice_h_z_distance_fn;
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_min=mw_height_above_mp-30;

			//mw_height_above_mp=mw_height_above_mp-subsection_slice_h_z_distance_fn;
			mw_height_above_mp=mw_height_above_mp-30;
			...
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->radial_distance_min=p_min;
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->radial_distance_max=p_max;
			...
			//	  subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->radial_distance_min)/2) +
			//	  subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->radial_distance_min;
            
		
			subsect_param_array[count_z+(number_zs_iterated*((int)ceil((float)5000/30)))]->midpoint_xy_plane_dist = 
				((subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->radial_distance_max -
				  subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->radial_distance_min)/2) +
				  subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->radial_distance_min;
               ...
               subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->mp_z_coord_height_above_midplane=0;                                                                                                                                                              
				...
				subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->mp_z_coord_height_above_midplane=
					((sqrt((subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->z_max)*(subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->z_max))+
					  sqrt((subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->z_min)*(subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->z_min)))/(2.0));
            }*/
              
            if ((subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_max+subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_min)==0){
               subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->mp_z_coord_height_above_midplane=0;                                                                                                                                                      
            } else{				
				subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->mp_z_coord_height_above_midplane=
					((sqrt((subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_max)*(subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_max))+
					  sqrt((subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_min)*(subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->z_min)))/(2.0));
            }
			...
			subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->volume=
				(
				...
				(				
				 ...
				 //(mw_angle_fn/360)*(pi_fn)*(subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->radial_distance_min)*
				 ((float)1/360)*(pi_fn)*(subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->radial_distance_min)*
			...
			total_volume=total_volume+subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->volume;
            ...
            if (number_density_type_fn==1){   
				//subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->num_stars=
				subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->num_stars=

					//whole_num_star((subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->volume)*
					whole_num_star((subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->volume)*
								  
								  ((n0_fn)*
								  //(exp(-subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->mp_z_coord_height_above_midplane/Z_thin_fn)+ 
								  (exp(-subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->mp_z_coord_height_above_midplane/Z_thin_fn)+ 
 	
									   //0.085*exp(-subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->mp_z_coord_height_above_midplane/Z_thick_fn))*
								       0.085*exp(-subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->mp_z_coord_height_above_midplane/Z_thick_fn))*

								  //exp(-subsect_param_array[count_z+(number_zs_iterated*z_subsections)]->midpoint_xy_plane_dist/h_R_fn)));
								  exp(-subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->midpoint_xy_plane_dist/h_R_fn)));
            ...
            if((subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->num_stars+num_stars_glbl)>=(num_stars_glbl+1)){
				subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->star_index_min=num_stars_glbl+1; 
            } else {
				subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->star_index_min=num_stars_glbl; 
            }
            ...
            num_stars_glbl=num_stars_glbl+subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->num_stars; 
            subsect_param_array[count_z+(number_zs_iterated* ((int)ceil((float)5000/30)))]->star_index_max=num_stars_glbl; 
	...
	count_z_glbl= (int)ceil((float)5000/30);
	count_xy_glbl= (int)ceil((float)15330/30);
...
void print_stats() {
	...
	int i = 0;
	#pragma omp parallel for private(i) schedule(dynamic) num_threads(1)
	for (i=1; i<=column_array[stats_counter]->num_stars; i++) {
	{
