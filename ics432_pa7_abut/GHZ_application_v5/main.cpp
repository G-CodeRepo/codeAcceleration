...
void copy_star2_into_star1_v2(long int length);	// alternate version
...
int main(int argc, char *argv[]) {
	...
	#pragma omp parallel shared(star) private (l, j, k) num_threads(1)	//num_threads(omp_get_max_threads())	//num_threads(2)
    {	
	for (l = 1; l<=count_xy_glbl; l++) {
		star = new star_cell [column_array[l]->num_stars+1];
		//loop that iterates through each cell in the column
		//#pragma omp parallel for collapse(2) private (j, k) schedule(dynamic) num_threads(1)
		for (j=column_array[l]->cell_min; j<=column_array[l]->cell_max; j++) {
			//#pragma omp parallel for shared(j) private(k) schedule(dynamic) num_threads(1)
		...
		if (num_in_process==1){
			...
			//if (num_in_process < 403) {
			//	copy_star2_into_star1_v2(ptr_1_range);
			//} else {
			...
			//if (num_in_process < 403) {
			//	copy_star2_into_star1_v2(ptr_2_range);
			//} else {
...
void process_SN_end_subsection() {
	...
	#pragma omp parallel shared(star1) private (i, j) num_threads(1)	//num_threads(omp_get_max_threads())	//num_threads(2)
    {	
	for (i=1; i<=ptr_1_range; i++) {
		...
		if(star1[i].sn==1 || star1[i].sn==2) {
			...
			//#pragma omp parallel for shared(i) private(j) schedule(dynamic) num_threads(1) //num_threads(omp_get_max_threads())
...
void process_SN() { 
	...
	#pragma omp parallel shared(star1) private (i, j, k) num_threads(1)	//num_threads(omp_get_max_threads())	//num_threads(2)
    {	
	for (i=1; i<=ptr_1_range; i++) {
	...
	#pragma omp parallel shared(star2) private (l, m) num_threads(1)	//num_threads(omp_get_max_threads())	//num_threads(2)
    {
...
int wrap_around_function(float x, float y, float z, float sterilization_distance) {
	...
	arc_length_original=(((acos(x/(sqrt((x*x)+(y*y)))))*(180/PI_fn))*(2*PI_fn/360))*sqrt((x*x)+(y*y));		
	arc_length_total=(((float)1)*(2*PI_fn/360))* sqrt((x*x)+(y*y));	
	//arc_length_ratio=arc_length_original/arc_length_total;
	arc_length_ratio=((((acos(x/(sqrt((x*x)+(y*y)))))*(180/PI_fn))*(2*PI_fn/360))*sqrt((x*x)+(y*y)))/((((float)1)*(2*PI_fn/360))* sqrt((x*x)+(y*y)));
    ...
    if (arc_length_ratio<=0.5) {
		...
		wrap_around.x_coord=(sqrt((x*x)+(y*y)))*(cos((((float)1)+(((float)1)*((acos(x/(sqrt((x*x)+(y*y)))))*(180/PI_fn))))*(PI_fn/180)));                
        wrap_around.y_coord=(sqrt((x*x)+(y*y)))*(sin((((float)1)+(((float)1)*((acos(x/(sqrt((x*x)+(y*y)))))*(180/PI_fn))))*(PI_fn/180)));
	...
	//outside_angle=((float)1)-angle;  //below the x-axis, from the origin
	outside_angle=((float)1)-((acos(x/(sqrt((x*x)+(y*y)))))*(180/PI_fn));  //below the x-axis, from the origin
...
long int running_total(int column) {
	long int total=0;
	int i = 0;
	//#pragma omp parallel shared(column_array) private(i) num_threads(1)
	//{
	...
	//	{
			...
			//memcpy((char*)total, (char*)(total+column_array[i]->num_stars), sizeof(long int));
			//} // end of pragma critical
	//	} // end of pragma
		} // end of for loop
	//}
...
void copy_into_star1(long int length) {
	...
	*/
	...
	#pragma omp critical
	{
	memcpy((char *)star1, (char *)star, sizeof(star_cell));
	}
}
...
void copy_into_star2(long int length) {
	...
	*/
	...
	#pragma omp critical
	{
	memcpy((char *)star2, (char *)star, sizeof(star_cell));
	}
}
...
void copy_star2_into_star1(long int length) {
	...
	*/
	...
	#pragma omp critical
	{
		memmove((unsigned char *)star1, (unsigned char *)star2, sizeof(star_cell));
	}
	
}

/*
void copy_star2_into_star1_v2(long int length) {
	star1 = new star_cell [length+1];
	#pragma omp critical
	{
		memmove((unsigned char *)star1, (unsigned char *)star2, sizeof(star_cell));
...
void copy_temp_into_star2(long int length) {
	...
	*/
	...
	#pragma omp critical
	{
	memcpy((unsigned char *)star2, (unsigned char *)star, sizeof(star_cell));
	}
...
void create_columns() {
	...
	int l = 0;
	#pragma omp parallel for private(l) schedule(dynamic) num_threads(1)
	for (l=1; l<=count_xy_glbl; l++) {
	{
